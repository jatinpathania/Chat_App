...
export class UserService {
  ...
  async setFriend({ id, otherId, status }) {
    ...
    if (status) {
      ...
      if (!secondUser.friends?.includes(id)) {
        await User.update(
  ...
  async setRequest({ id, otherId, status }) {
    ...
    if (status) {
      ...
      // Accept friend request - remove from requests AND add to friends
      // First remove from requests
      await User.update(
        { requests: sequelize.fn('array_remove', sequelize.col('requests'), id) },
        { where: { id:otherId } }
      );
      
      // Then add both as friends (if not already friends)
      if (!firstUser.friends?.includes(otherId)) {
        await User.update(
          { friends: sequelize.fn('array_append', sequelize.col('friends'), otherId) },
          { where: { id } }
